...
export const ReadingCompletionQuiz: React.FC<ReadingCompletionQuizProps> = React.memo(
  ({
    ...
    // Sync when cachedQuiz arrives or updates from parent
    useEffect(() => {
    ...
    useEffect(() => {
      ...
      const targetKey = `${articleId || articleTitle}-${cefrLevel}`;
      if (hasFetchedRef.current === targetKey) {
        return;
      }
      ...
      const fetcher = onGetQuiz
        ...
        : apiReadingRepository.generateQuiz(
            articleId,
            articleTitle,
            articleContent,
            keywords,
      ...
      fetcher
        ...
        .catch((err) => {
          logger.warn("Quiz fetch fallback trigger", err);
          if (isMountedRef.current) {
            setLoading(false);
          }
        });
    ...
    return (
      <div className="w-full max-w-[600px] flex flex-col justify-between select-none animate-[fadeSlideUp_0.4s_ease-out_both]">
        ...
        {isAnswered && (
          ...
          >
            <span className="text-[#A27FF3] text-sm shrink-0 mt-0.5" aria-hidden="true">
              ✦
            </span>
