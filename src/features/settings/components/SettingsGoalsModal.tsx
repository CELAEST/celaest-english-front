...
export const SettingsGoalsModal: React.FC<SettingsGoalsModalProps> = ({ isOpen, currentGoal, onSelectGoal, onClose }) => {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="goals-modal-title" className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-xl animate-[fadeIn_0.18s_ease-out]" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl sm:rounded-[28px] border border-white/10 bg-[#0c0c14]/90 p-5 sm:p-7 shadow-[0_24px_64px_rgba(0,0,0,0.55),0_0_0_1px_rgba(255,255,255,0.04)] backdrop-blur-2xl flex flex-col max-h-[88dvh] sm:max-h-[80vh] animate-[scaleUp_0.22s_cubic-bezier(0.16,1,0.3,1)_both]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex flex-col gap-2 pt-4 sm:pt-5 max-h-[70dvh] sm:max-h-[60vh] overflow-y-auto no-scrollbar [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden pr-1 -mr-1">
          {GOALS.map((g) => {
            ...
            return (
              <button key={g.id} type="button" aria-pressed={isSelected} onClick={async () => { await onSelectGoal(g.id); onClose(); }} className={`group flex items-center gap-3 sm:gap-4 rounded-xl sm:rounded-2xl border p-3.5 sm:p-4 text-left transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20 ${isSelected ? "border-white/15 bg-white text-zinc-900 shadow-sm" : "border-white/5 bg-white/[0.03] hover:border-white/10 hover:bg-white/[0.06]"}`}>
