  ...
  return (
    <article className="relative flex flex-col justify-between overflow-hidden rounded-3xl border border-[#111220] bg-[#05060c] p-5 sm:p-6 lg:p-7 shadow-2xl backdrop-blur-xl select-none min-h-[320px] max-h-[460px] h-full animate-[slideInLeft_0.5s_cubic-bezier(0.22,1,0.36,1)_both]">
      ...
      <div className="flex items-center justify-between shrink-0">
        <span className="text-[11px] sm:text-xs font-semibold tracking-[0.18em] text-[#A27FF3]/80 uppercase">
          FRONT
        </span>
        <button
          onClick={onBookmark}
          aria-label="Bookmark card"
          className="text-[#999a9b] transition-colors hover:text-[#A27FF3] cursor-pointer"
        >
          <svg
      ...
      {/* Skill Tag */}
      <div
        className="mt-3 sm:mt-4 inline-flex items-center gap-1.5 sm:gap-2 rounded-full bg-white/[0.05] px-3 py-1 sm:py-1.5 text-[10px] sm:text-xs font-medium tracking-wide text-white/90 self-start border border-white/[0.06] shrink-0 animate-[fadeSlideUp_0.5s_ease-out_both]"
        style={{ animationDelay: "150ms" }}
      >
        <svg
          className="h-4 w-4 sm:h-[18px] sm:w-[18px] text-[#A27FF3] shrink-0"
          viewBox="0 0 24 24"
          fill="currentColor"
        >
      ...
      {/* You Said Section */}
      <div
        className="mt-4 sm:mt-5 shrink-0 animate-[fadeSlideUp_0.5s_ease-out_both]"
      ...
      {/* Divider */}
      <hr
        className="my-4 sm:my-5 border-[#111220] shrink-0 animate-[fadeIn_0.4s_ease-out_both]"
        style={{ animationDelay: "350ms" }}
      />

      {/* Better Way Section */}
      <div
        className="shrink-0 animate-[fadeSlideUp_0.5s_ease-out_both]"
      ...
      <div
        className="mt-auto flex items-center justify-center gap-1.5 sm:gap-2 pt-4 sm:pt-5 text-[11px] sm:text-[13px] text-[#999a9b] shrink-0 animate-[fadeIn_0.5s_ease-out_both]"
...
const HighlightWord: React.FC<{ sentence: string; word: string; color: string }> = ({
  sentence,
  word,
  color,
}) => {
