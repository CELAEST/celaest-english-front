  ...
  return (
    <article
      ...
      className={`relative flex flex-col justify-between overflow-hidden rounded-3xl border border-[#111220] bg-[#05060c] p-5 sm:p-6 lg:p-7 shadow-2xl backdrop-blur-xl transition-all duration-700 ease-out select-none animate-[slideInRight_0.5s_cubic-bezier(0.22,1,0.36,1)_both] min-h-[320px] max-h-[460px] h-full ${
        revealed
          ? "opacity-100 blur-0 scale-100 select-text"
          : "pointer-events-none opacity-50 blur-[8px] scale-[0.98]"
      }`}
    >
      {/* Header */}
      <div className="flex items-start justify-between shrink-0">
        ...
        <button
          ...
          className="text-[#999a9b] transition-colors hover:text-white cursor-pointer"
        >
      ...
      <div className="mt-2.5 sm:mt-3 flex items-start gap-3 shrink-0">
        <button
          ...
          className="mt-0.5 text-[#A27FF3] cursor-pointer hover:scale-110 transition-transform"
        >
          <svg
      ...
      <div className="flex items-start gap-3 shrink-0">
        <svg
          className="mt-0.5 h-4.5 w-4.5 sm:h-5 sm:w-5 shrink-0 text-[#A27FF3]"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth={1.6}
        ...
        <div>
          ...
          <p className="mt-1 leading-relaxed text-white/90 text-xs sm:text-sm">{explanationText}</p>
        </div>
      </div>
