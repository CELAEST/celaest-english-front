# PROJECT MANIFEST: Lingua (CELAEST English)
...
## 3. Product Anti-Goals (What We Will NEVER Do)
...
## 4. Virtual Senior Expert Council (Governance Rule)

Every decision in this project is subjected to the scrutiny of 8 specialized virtual experts:

1. **Product Designer (Senior)**: Guarantees radical simplicity, workflow clarity, and user focus.
2. **UI Designer (Senior)**: Enforces pixel perfection, color token integrity, visual hierarchy, and typography elegance.
3. **UX Researcher**: Validates every interaction against actual human cognitive load and learning science.
4. **Frontend Architect**: Enforces clean architecture, strict state isolation, modularity, and high performance.
5. **Design System Engineer**: Enforces 100% component reusability and token compliance.
6. **Accessibility Expert**: Ensures WCAG 2.1 AA/AAA compliance, screen reader support, and full keyboard navigation.
7. **Motion Designer**: Curates subtle, high-end micro-animations and glowing particle transitions.
8. **AI Product Designer**: Ensures the UI seamlessly displays streaming AI intelligence, audio waveforms, and learning graph adaptations.

---

## 5. Non-Negotiable Engineering Rules

1. **Zero Unjustified Code**: Every component and line of logic must have a documented UX or technical justification.
2. **Design System Supremacy**: UI components must strictly consume CSS variable design tokens defined in `04_DESIGN_SYSTEM.md`.
3. **Feature-Driven Architecture**: Code must be modularized by domain features (`features/conversation`, `features/memory`, `features/reading`, `features/writing`, `features/interview`).
4. **Performance Benchmark**: First Contentful Paint < 0.8s, 60fps animations, zero main-thread audio lag.
