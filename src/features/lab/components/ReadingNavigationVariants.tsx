...
export const ReadingNavigationVariants: React.FC = () => {
  const [activeVariant, setActiveVariant] = useState<'split-minimal' | 'integrated-bar' | 'floating-edges'>('split-minimal');
  ...
  return (
    <div className="flex flex-col space-y-4 p-6 rounded-3xl bg-[#070714]/80 border border-white/[0.06] backdrop-blur-2xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h3 className="text-base font-semibold text-white tracking-wide">Reading Navigation Lab</h3>
          <p className="text-xs text-[#8a8a9e]">Comparing container-less, split, and integrated bottom bar UX/UI designs</p>
        </div>
          ...
          <div className="w-full max-w-[580px] mx-auto flex flex-col space-y-3">
            ...
            <div className="w-full flex items-center justify-between pt-1">
              ...
              <span className="text-xs font-mono tracking-widest text-[#7e8096]">
                <strong className="text-white font-medium">{String(currentPage).padStart(2, '0')}</strong> / {String(totalPages).padStart(2, '0')}
              </span>
          ...
          <div className="w-full max-w-[580px] mx-auto flex flex-col space-y-3">
            ...
            <div className="flex items-center justify-between text-xs text-[#8a8a9e]">
              ...
              <div className="flex items-center space-x-3">
                <button onClick={prev} className="hover:text-white p-1 cursor-pointer">← Prev</button>
                <span className="text-[#A27FF3] font-mono">0{currentPage} / 0{totalPages}</span>
                <button onClick={next} className="hover:text-white p-1 cursor-pointer">Next →</button>
              </div>
